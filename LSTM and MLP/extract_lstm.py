...
class GenreFeatureDataVal:   
    preprocessed_data = './extract/dataTest'
    train_X = train_Y = None
    test_X = test_Y = None

    def load_and_save_data(self, path, path_id):
        files = self.path_to_audiofiles(path)
...
genre_features = GenreFeatureDataVal()  
with open(filepath) as fp:  
   line = fp.readline().strip()
   count = 0
   while line:
       genre_features.load_and_save_data(line, count)
       line = fp.readline().strip()
       count += 1