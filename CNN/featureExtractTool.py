################################################################################
#  Toolbox for extracting the segment of melspectrogram from song as patterns. #
################################################################################
...
def generate_test_set(a_PathToSongDir, a_Label, a_SizeOfWindow, a_NumOfOverlapSample, a_NameOfOutput):
    ...
    #################################################################################
    t_counter = 0
    f = open('misclassNonprog2Prog.txt','w')
    with open('classNonprogAsProg.pkl', "rb") as t_PKLFile:
        t_listtttt = pickle.load(t_PKLFile)
    ...
    for i, t_SongName in enumerate(t_AllFiles):
        if t_SongName.endswith(".mp3"):
            ...
            if t_counter in t_listtttt:
                f.write(f'{t_SongName}\n')
            ...
            """
        print("Label:", a_Label, " Processed ", t_SongName, " No.", i+1, "/", len(t_AllFiles), "  {0:.2%}".format((i+1)/len(t_AllFiles)))
...
def generate_patterns_for_all_songs_in_dir(a_PathToSongDir, a_Label, a_SizeOfWindow, a_NumOfOverlapSample, a_NameOfOutputPatterns):
    ...
    for i, t_SongName in enumerate(t_AllFiles):
        ...
        print("Current Pattern shape = ", t_AllPatterns.shape)
...
def get_raw_song(a_PathToSingleSong):
    ...
    t_RawFullSong, t_SR = librosa.load(a_PathToSingleSong, offset=30, mono=True, sr=22050)
...
def extract_mel(a_RawSong, a_SR):
    ...
    #librosa.display.specshow(librosa.power_to_db(t_Mel, ref=np.max), y_axis='mel', fmax=8000, x_axis='s', hop_length=2)
    #librosa.display.specshow(librosa.power_to_db(t_Mel, ref=np.max), y_axis='mel', fmax=8000, x_axis='time')
